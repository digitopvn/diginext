...
class RelationIdLoader {
            ...
            relatedEntities.forEach((relatedEntity) => {
                entityRelationIds.forEach((relationId) => {
                    const relatedEntityMatched = columns.every((column) => {
                        return column.compareEntityValue(relatedEntity, relationId[DriverUtils_1.DriverUtils.buildAlias(this.connection.driver, column.entityMetadata.name +
                            "_" +
    ...
    loadForManyToMany(relation, entities, relatedEntities) {
        ...
        columns.forEach((column) => {
            const columnName = DriverUtils_1.DriverUtils.buildAlias(this.connection.driver, column.referencedColumn.entityMetadata.name +
                "_" +
    ...
    loadForManyToOneAndOneToOneOwner(relation, entities, relatedEntities) {
        ...
        const qb = this.connection.createQueryBuilder();
        relation.entityMetadata.primaryColumns.forEach((primaryColumn) => {
            const columnName = DriverUtils_1.DriverUtils.buildAlias(this.connection.driver, primaryColumn.entityMetadata.name +
                "_" +
                primaryColumn.propertyPath.replace(".", "_"));
