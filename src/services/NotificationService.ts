	...
	async send(data: SendNotificationData, options?: IQueryOptions) {
		...
		if (isArray(data.to)) {
			...
			return Promise.all(
				...
				.then((notifications) => {
					...
					notifications.map(({ owner: recipientId }) => {
						socketIO?.to(MongoDB.toString(recipientId)).emit("notification", { action: "new", message: "You have new notification" });
					});
			...
			return this.create(notiData, options)
				.then((noti) => {
					// emit websocket to clients
					socketIO?.to(MongoDB.toString(noti.owner)).emit("notification", { action: "new", message: "You have new notification" });
					return noti;
				})
				.catch((e) => {
					console.error(`Unable to send notification:`, e);
