...
export class MonitorServiceService {
			...
			const ls = await Promise.all(
				clusters.map(async (cluster) => {
					...
					let nsList = namespace
						? ((await ClusterManager.getServices(namespace, { context, output: options?.output })) as KubeService[])
						: await ClusterManager.getAllServices({ context, output: options?.output });
	...
	async delete(params: MonitoringQueryParams) {
		...
		const svc = name
			...
			: await ClusterManager.deleteServiceByFilter(namespace, { context, filterLabel: objectToFilterLabels(params.labels) });
